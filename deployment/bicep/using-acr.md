# Documentation on using ACR
We have to use [Azure Container Registry (ACR)](https://learn.microsoft.com/en-us/azure/) to comply with security policies for:
- Kubernetes control plane images.
- Nvidia GPU operators or device plugins.
- Application and model wrapper container images.

To setup your own ACR follow the instructions in [ACR README](../acr/README.md).
For ACR login and credential management, see [Deployment README](../README.md#azure-container-registry-acr).
...
sudo kubeadm init \
  ...
  --image-repository="$ACR_URL"
```

You might have the `401 unauthorize` error, try adding this to `/etc/containerd/config.toml`:

```toml
[plugins."io.containerd.grpc.v1.cri".registry.configs."$ACR_URL.azurecr.io".auth]
    username = "$ACR_NAME"
    password = "<password>"
```
...
ACR_NAME="abcd"  # TODO fill
ACR_FULL_NAME="$ACR_NAME-xyz"  # TODO fill
ACR_URL="$ACR_FULL_NAME.azurecr.io"  # TODO fill
ACR_USERNAME="abcd"  # TODO fill
ACR_EMAIL="abcd@microsoft.com"  # TODO fill
...
Just point the address to ACR.


## Azure Kubernetes Services
We can attach the ACR to an AKS cluster:
```bash
