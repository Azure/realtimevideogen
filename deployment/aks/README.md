...
- **Spot VM evicted**: Spot VMs may be evicted at any time. Re-run `az aks nodepool scale` to restore the node. After re-scaling a MIG node you must repeat the [MIG Setup Guide](../k8s/MIG.md) since MIG state does not persist across evictions. If `OverconstrainedAllocationRequest` errors occur, retry after a few minutes — Spot capacity is first-come first-served
- **Pods stuck in ContainerCreating after node reboot**: After a VMSS restart, the Azure CNS (Container Networking Service) takes ~30–60 seconds to start. Pods scheduled during this window fail with `connection refused` on `localhost:10090`. Wait and they will self-heal, or delete and recreate them
- **LoadBalancer stuck in Pending**: Verify the public IP exists and the AKS identity has Network Contributor role on the resource group
- **Cannot reach LoadBalancer services (e.g. StreamWise at :8081)**: Azure LoadBalancer performs DNAT: inbound traffic on the Service port (e.g. 8081) is translated to a NodePort (30000-32767) on the node private IP. The subnet NSG evaluates the post-DNAT packet, so a rule matching the public IP on ports 8000-9000 alone is not sufficient. The Bicep template now includes an AllowK8sNodePorts rule (priority 102) that allows TCP 30000-32767 from Internet to VirtualNetwork. If a corporate policy replaces or overrides this NSG, re-add it manually. Note: traffic from CorpNet works regardless (NRMS-Rule-103 allows all from CorpNetPublic). Azure evaluates both the subnet NSG and the NIC-level NSG (in the MC_ resource group) - traffic must be allowed by both.
- **Secret errors**: Verify HF token is correctly configured
- **ACR role assignment fails on redeployment**: The role already exists. Attach ACR manually. If an in-progress AKS operation blocks this, wait 1-2 minutes and retry
- **Let's Encrypt fails with Timeout during connect**: Corporate Azure subscriptions have NRMS rules (priorities 101-109) that block inbound Internet traffic on port 80 at the Azure platform level. Even though the subnet NSG allows port 80, the ACME HTTP-01 validation from Let's Encrypt servers (external Internet) cannot reach the solver pod. This cannot be fixed with NSG rules alone - it requires a subscription policy change. Use the self-signed certificate instead (accept the browser warning or use curl -k). To retry later: bash deployment/aks/setup-letsencrypt.sh
- **Key Vault certificate not found after Bicep**: The self-signed cert creation in Key Vault can race with cluster provisioning. quick-deploy.sh now waits up to 2 minutes. If it still does not appear, the script falls back to openssl
- **VMSS instance IDs not starting at 0**: After Spot eviction and re-provision, VMSS instance IDs increment. Always query the actual ID
- **Federated identity credential error during Bicep**: Transient race condition. The AKS cluster is created successfully - re-run the Bicep deployment or proceed without them (only needed for workload identity / CSI secret provider)
...
A self-contained end-to-end deployment script is available at
[`deployment/aks/quick-deploy.sh`](quick-deploy.sh).
Edit the configuration variables at the top, then run from the repository root:
