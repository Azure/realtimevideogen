...
- **LoadBalancer stuck in Pending**: Verify the public IP exists (`az network public-ip show -g $AZ_RESOURCE_GROUP --name aks-pods-public-ip`) and the AKS identity has Network Contributor role on the resource group. **Important:** `RESOURCE_GROUP_NAME` must be set to the *main* resource group (`$AZ_RESOURCE_GROUP`) where `aks.bicep` creates `aks-pods-public-ip` — **not** the `MC_` managed-cluster resource group. Using the `MC_` group causes `cannot find public IP` errors because the IP does not exist there.
