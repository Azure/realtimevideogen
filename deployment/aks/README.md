...
- **Let's Encrypt fails with "Timeout during connect"**: Corporate Azure subscriptions have NRMS rules that restrict inbound Internet traffic. Add a NodePort allow rule: `az network nsg rule create -g $AZ_RESOURCE_GROUP --nsg-name aks-node-subnet-nsg --name AllowK8sNodePorts --priority 102 --direction Inbound --access Allow --protocol Tcp --source-address-prefixes Internet --destination-address-prefixes VirtualNetwork --destination-port-ranges 30000-32767`. If it still fails, the subscription may have Azure-level restrictions blocking non-CorpNet traffic — use the self-signed certificate instead
- **VMSS instance IDs not starting at 0**: After Spot eviction and re-provision, VMSS instance IDs increment. Always query the actual ID: `az vmss list-instances -g $MC_RESOURCE_GROUP -n <vmss> --query '[0].instanceId' -o tsv`
- **Federated identity credential error during Bicep**: The `ConcurrentFederatedIdentityCredentialsWritesForSingleManagedIdentity` error is a transient race condition. The AKS cluster is created successfully — re-run the Bicep deployment or proceed without them (only needed for workload identity / CSI secret provider)
