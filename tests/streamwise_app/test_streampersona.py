...
async def test_app(test_app: Quart) -> None:
    ...
    assert "StreamPersona" in response_html
...
async def test_files(test_app: Quart) -> None:
    ...
    if not os.path.exists("/tmp/streampersona"):
        os.makedirs("/tmp/streampersona", exist_ok=True)
    ...
    response_json = await response.get_json()
    assert response_json == {"error": "File '/tmp/streampersona/testfile.txt' not found"}
