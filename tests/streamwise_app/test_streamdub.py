...
@pytest.mark.asyncio
async def test_detect_speaker_gender_male() -> None:
    """detect_speaker_gender returns 'male' when LLM responds with 'male'."""
    with patch.dict(sys.modules, {**mock_modules, **scene_mocks_base}):
        with temp_sys_path("apps", "apps/streamdub"):
            from apps.streamdub.streamdub_job import StreamDubJob as _StreamDubJob
            from apps.scene import SceneSegment

    job = _StreamDubJob(job_id="test_gender_male", service_manager=AsyncMock())
    scene = SceneSegment(
        scene_id=0, start_frame=0, end_frame=30,
        start_sec=0.0, end_sec=1.0,
    ...
    job.gen = MagicMock()
    job.gen.gen_text = AsyncMock(return_value="male")
    job.gen.stop = AsyncMock()
    gender = await job.detect_speaker_gender(scene)
    assert gender == "male"
    await job.close()


@pytest.mark.asyncio
async def test_detect_speaker_gender_female() -> None:
    """detect_speaker_gender returns 'female' when LLM responds with 'female'."""
    with patch.dict(sys.modules, {**mock_modules, **scene_mocks_base}):
        with temp_sys_path("apps", "apps/streamdub"):
            from apps.streamdub.streamdub_job import StreamDubJob as _StreamDubJob
            from apps.scene import SceneSegment

    job = _StreamDubJob(job_id="test_gender_female", service_manager=AsyncMock())
    scene = SceneSegment(
        scene_id=0, start_frame=0, end_frame=30,
        start_sec=0.0, end_sec=1.0,
        transcript="She walked in.",
    )
    job.gen = MagicMock()
    job.gen.gen_text = AsyncMock(return_value="female")
    job.gen.stop = AsyncMock()
    gender = await job.detect_speaker_gender(scene)
...
@pytest.mark.asyncio
async def test_detect_speaker_gender_fallback() -> None:
    """detect_speaker_gender falls back to 'female' on LLM error or unrecognised response."""
    with patch.dict(sys.modules, {**mock_modules, **scene_mocks_base}):
        with temp_sys_path("apps", "apps/streamdub"):
            from apps.streamdub.streamdub_job import StreamDubJob as _StreamDubJob
            from apps.scene import SceneSegment

    job = _StreamDubJob(job_id="test_gender_fallback", service_manager=AsyncMock())

    # Empty transcript → should default without calling LLM
    scene_empty = SceneSegment(
        scene_id=0, start_frame=0, end_frame=30, start_sec=0.0, end_sec=1.0,
    )
    job.gen = MagicMock()
    job.gen.stop = AsyncMock()
    job.gen.gen_text = AsyncMock(return_value="unknown_value")
    assert await job.detect_speaker_gender(scene_empty) == "female"

    # LLM raises an exception → should return "female"
    scene_with_text = SceneSegment(
        scene_id=1, start_frame=0, end_frame=30, start_sec=0.0, end_sec=1.0,
        transcript="Some text.",
    )
    job.gen.gen_text = AsyncMock(side_effect=RuntimeError("service unavailable"))
    assert await job.detect_speaker_gender(scene_with_text) == "female"
