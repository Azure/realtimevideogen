"""
Hunyuan FramePack F1 wrapper class.
"""
import sys
import logging
import time
...
class HunyuanFramepackF1Generation(HunyuanFramePackBase):
    """Handle video generation using the Hunyuan Framepack F1 model."""
    def __init__(
        self,
        ...
        super().__init__(
            ...
            framepack_model_name="lllyasviel/FramePack_F1_I2V_HY_20250503",
    ...
    async def generate(
        ...
        latent_window_size: int = 9,  # latent frames for every inference window: 9->36 pixel frames -> 1.2 seconds
        cfg: float = 1.0,  # prompt guidance scale
        ...
        https://raw.githubusercontent.com/lllyasviel/FramePack/refs/heads/main/demo_gradio_f1.py
        ...
        if self.rank == 0:
            logging.info(
                f"[{self.rank}] Length:{video_seconds:.3f} seconds "
                f"Frames:{num_frames} Frames/iteration:{num_frames_it} "
                f"Iterations:{total_latent_sections} FPS:{self.FPS} CFG:{cfg}.")

        self.running = True  # Mark running to avoid concurrent calls

        try:
            # Text encoder
            llama_vec, llama_attention_mask, clip_l_pooler, llama_vec_n, \
                llama_attention_mask_n, clip_l_pooler_n = self._encode_text(
                    gen_timer,
                    prompt,
                    neg_prompt,
                    cfg)
            ...
            history_latents = torch.cat([
                history_latents,
                start_latent.to(history_latents)
            ], dim=2)
            ...
            for it in range(total_latent_sections):
                ...
                clean_latent_indices_start, clean_latent_4x_indices, clean_latent_2x_indices, \
                    clean_latent_1x_indices, latent_indices = indices.split([1, 16, 2, 1, latent_window_size], dim=1)
                ...
                if self.rank == 0:
                    ...
                    logging.info(
                        f"[{self.rank}] lat_frames:{lat_frames} frames:{cur_frames} "
            ...
            if self.rank != 0:
                return None  # other workers do not need to return anything or VAE decode  # type: ignore[return-value]
            ...
            # [1, 16, lat_frames, lat_h, lat_w] -> [1, 3, 1+2+16, h, w] ([1, 16, 37, 68, 88]->[1, 3, 145, 544, 704])
            gen_timer.start("vae_decoder")
            if history_pixels is None:
                # This is the common path
                history_pixels = await asyncio.to_thread(
                    vae_decode,
                ...
                current_pixels = await asyncio.to_thread(
                    vae_decode,
                    real_history_latents[:, :, -section_latent_frames:],
                    self.vae)
                current_pixels = current_pixels.cpu()
                history_pixels = soft_append_bcthw(history_pixels, current_pixels, overlapped_frames)
            gen_timer.end("vae_decoder")

            if self.rank == 0:
                logging.info(
                    f"[{self.rank}] VAE decode. Latent:{real_history_latents.shape}->Pixel:{history_pixels.shape}.")
