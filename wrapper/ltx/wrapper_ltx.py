...
class LTXVideoGeneration(ModelGeneration):
    ...
    def round_to_nearest_resolution_acceptable_by_vae(
        ...
        height = height - (height % self.pipeline.vae_spatial_compression_ratio)  # type: ignore[union-attr]
        width = width - (width % self.pipeline.vae_spatial_compression_ratio)  # type: ignore[union-attr]
        return height, width
    ...
    async def get_rest_args(
        ...
        assert isinstance(img_base64, str), f"Expected 'img' to be a string, got {type(img_base64)}"
