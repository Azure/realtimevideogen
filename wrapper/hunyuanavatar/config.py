...
from hymm_sp.constants import TEXT_ENCODER_PATH
from hymm_sp.constants import TOKENIZER_PATH
from hymm_sp.constants import PROMPT_TEMPLATE
from hymm_sp.constants import TEXT_PROJECTION
from hymm_sp.constants import PRECISIONS
...
def add_extra_models_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
...
def add_denoise_schedule_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    group = parser.add_argument_group(title="Denoise schedule")
    group.add_argument("--flow-shift-eval-video", type=float, default=None,
                       help="Shift factor for flow matching schedulers when using video data.")
    group.add_argument("--flow-reverse", action="store_true", default=True,
                       help="If reverse, learning/sampling from t=1 -> t=0.")
    group.add_argument("--flow-solver", type=str, default="euler", help="Solver for flow matching.")
    group.add_argument("--use-linear-quadratic-schedule", action="store_true",
    ...
    group.add_argument("--linear-schedule-end", type=int, default=25,
...
def sanity_check_args(args: argparse.Namespace) -> argparse.Namespace:
    # VAE channels
    ...
    if vae_channels != args.latent_channels:
        raise ValueError(f"Latent ({args.latent_channels}) must match VAE ({vae_channels}).")
    return args
