...
class HunyuanVideoSampler(Inference):
    def __init__(self, args: Any, vae: Any, vae_kwargs: Any, text_encoder: Any, model: Any,
        ...
        self.pipeline = load_diffusion_pipeline(
            ...
            device=self.device)
        self.pipeline.scheduler.config.shift = 1.0
        logging.info('Loaded HunyuanVideoSampler successfully... ')
    ...
    def predict(self,
        ...
        if fps.dim() == 0:
            ...
            print(f"fps was not a scalar, keeping as: {fps}, dim: {fps.dim()}")
        audio_prompts = batch["audio_prompts"].to(self.device)
        weight_dtype = audio_prompts.dtype

        # audio_prompts = [
        # encode_audio(wav2vec, audio_feat.to(dtype=wav2vec.dtype), fps.item(), num_frames=batch["audio_len"])
        # for audio_feat in audio_prompts
        # ]
        audio_prompts = encode_audio(wav2vec, audio_prompts.to(dtype=wav2vec.dtype),
                                     fps.item(), num_frames=batch["audio_len"])
        # audio_prompts = torch.cat(audio_prompts, dim=0).to(device=self.device, dtype=weight_dtype)
        audio_prompts = audio_prompts.to(device=self.device, dtype=weight_dtype)
        if audio_prompts.shape[1] <= 129:
            audio_prompts = torch.cat([audio_prompts, torch.zeros_like(
                audio_prompts[:, :1]).repeat(1, 129 - audio_prompts.shape[1], 1, 1, 1)], dim=1)
        else:
            audio_prompts = torch.cat([audio_prompts, torch.zeros_like(
                audio_prompts[:, :1]).repeat(1, 5, 1, 1, 1)], dim=1)
        ...
        pixel_value_ref = batch['pixel_value_ref'].to(self.device)  # (b f c h w) range from [0,255]
        # convert pixel_value_ref from (b c h w) to (b f c h w) with f = 1
        pixel_value_ref = rearrange(pixel_value_ref, "b c h w -> b 1 c h w")  # adding the f dimension
        ...
        pixel_value_llava = rearrange(pixel_value_llava, "b c h w -> b 1 c h w")  # adding the f dimension
