...
class VibeVoiceForConditionalGeneration(VibeVoicePreTrainedModel):
    ...
    def tie_weights(self) -> None:
        ...
        if getattr(self.config.decoder_config, 'tie_word_embeddings', False):
            ...
            logger.info("✅ Tied input and output embeddings using standard assignment.")
        else:
            logger.info("ℹ️  tie_word_embeddings is False, not tying weights.")
            ...
            with torch.no_grad():
                ...
                if torch.isnan(self.model.speech_scaling_factor) or torch.isnan(self.model.speech_bias_factor):
                    ...
                    if dist.is_available() and dist.is_initialized():
                        ...
                        logger.info(
                            f"Speech scaling factor (distributed): {self.model.speech_scaling_factor}, "
                            f"bias factor: {self.model.speech_bias_factor}", flush=True)
                    else:
                        # Single process case
                        self.model.speech_scaling_factor.copy_(scaling_factor)
                        self.model.speech_bias_factor.copy_(bias_factor)
                        logger.info(
                            f"Speech scaling factor (single process): {self.model.speech_scaling_factor}, "
                            f"bias factor: {self.model.speech_bias_factor}", flush=True)
        ...
        if speeches_loss_input is not None:
            ...
            if speech_tensors is not None:
                if semantic_speech_all_connect_features is not None:
                    ...
                    x[acoustic_input_mask] = speech_all_connect_features[speech_masks]
                mask = speeches_loss_input.unsqueeze(-1) & speech_masks
                speech_features = speech_all_features[mask]  # only part audio need diffuse
                speech_connect_features = speech_all_connect_features[mask]
        else:
