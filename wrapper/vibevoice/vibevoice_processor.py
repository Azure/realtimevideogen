...
from modular_vibevoice_text_tokenizer import VibeVoiceTextTokenizerFast
...
class VibeVoiceProcessor:
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.system_prompt = (
            " Transform the text provided by various speakers into speech output, utilizing the "
    ...
    def __call__(
        self,
        text: Optional[Union[str, List[Any], TextInput, PreTokenizedInput]] = None,
    ...
    def _process_single(
        ...
        full_tokens += (
            self.tokenizer.encode(' Speech output:\n', add_special_tokens=False)
    ...
    def prepare_speech_inputs(
        ...
        # vae_tok_seqlens = [math.ceil(s.shape[0] / self.speech_tok_compress_ratio)
        # if s.ndim == 1 else s.shape[0] for s in speech_inputs]
    ...
    def model_input_names(self) -> List[str]:
        ...
        """
        if not self.tokenizer or not self.audio_processor:
            raise ValueError("Tokenizer or audio processor is not initialized.")
        tokenizer_input_names = self.tokenizer.model_input_names
        audio_processor_input_names = self.audio_processor.model_input_names
        return list(dict.fromkeys(
            tokenizer_input_names + audio_processor_input_names + ["speech_inputs", "speech_input_mask"]))
    ...
    def save_audio(
        ...
        if not self.audio_processor:
            raise ValueError("Audio processor is not initialized.")
