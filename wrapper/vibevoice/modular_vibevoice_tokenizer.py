...
class VibeVoiceTokenizerStreamingCache:
    ...
    def __init__(self) -> None:
        self.cache: Dict[Tuple[str, int], torch.Tensor] = {}  # Dict mapping (layer_id, sample_idx) to state tensor
...
class SConv1d(nn.Module):
    ...
    def _forward_streaming(
        ...
        if cached_states is None:
            ...
            if self.context_size > 0:
                ...
                logger.debug(f"Initialized cache with shape: {cached_states.shape}, context_size={self.context_size}")
            else:
                cached_states = torch.zeros(B, C, 0, device=x.device, dtype=x.dtype)
                logger.debug("No context needed (kernel_size=stride)")
        ...
        logger.debug(
            f"Input shape: {x.shape}, Cache shape: {cached_states.shape}, Combined: {input_with_context.shape}")
        ...
        if self.context_size > 0:
            ...
            logger.debug(f"New cache shape: {new_cache.shape}")
    ...
    def _forward_non_streaming(self, x: torch.Tensor, debug: bool = False) -> torch.Tensor:
        ...
        logger.debug(f"Input shape: {x.shape}, padding_total={padding_total}, extra_padding={extra_padding}")
...
class SConvTranspose1d(nn.Module):
    ...
    def _forward_streaming(
        ...
        if cached_input is None:
            ...
            logger.debug("Initialized empty cache for transposed conv")

        # Concatenate cached input with new input
        full_input = torch.cat([cached_input, x], dim=2)

        logger.debug(f"Input shape: {x.shape}, Cache shape: {cached_input.shape}, Combined: {full_input.shape}")

        # First chunk or debug mode - use uncompiled version
        full_output = self.convtr(full_input)

        logger.debug(f"Full transposed conv output shape: {full_output.shape}")
        ...
        logger.debug(f"After unpadding: {full_output.shape}")
        ...
        logger.debug(f"Final streaming output shape: {output.shape}")
    ...
    def _forward_non_streaming(self, x: torch.Tensor, debug: bool = False) -> torch.Tensor:
        ...
        logger.debug(f"After transposed conv: {y.shape}")
