...
from modeling_vibevoice import VibeVoiceModel
from modeling_vibevoice import VibeVoicePreTrainedModel
from audio_streamer import AudioStreamer
from audio_streamer import AsyncAudioStreamer
...
class VibeVoiceForConditionalGenerationInference(VibeVoicePreTrainedModel, GenerationMixin):
    ...
    @property
    def speech_scaling_factor(self) -> torch.Tensor:
        return self.model.speech_scaling_factor

    @property
    def speech_bias_factor(self) -> torch.Tensor:
        return self.model.speech_bias_factor

    @property
    def acoustic_tokenizer(self) -> Any:
        return self.model.acoustic_tokenizer

    @property
    def semantic_tokenizer(self) -> Any:
        return self.model.semantic_tokenizer

    @property
    def acoustic_connector(self) -> nn.Module:
        return self.model.acoustic_connector

    @property
    def semantic_connector(self) -> nn.Module:
        ...
        for step in progress_bar:
            ...
            if (next_tokens == generation_config.eos_token_id).any():
                ...
                if new_eos_indices.numel() > 0:
                    ...
                    if verbose:
                        logger.info(f"Samples {new_eos_indices.tolist()} reached EOS token at step {step + 1}.")
                    if audio_streamer is not None:
            ...
            if diffusion_indices.numel() > 0:
                ...
                if non_diffusion_mask.any():
                    ...
                    for i, (sample_idx, start_idx) in enumerate(zip(
    ...
    def sample_speech_tokens(
        ...
        neg_condition: torch.FloatTensor,
        cfg_scale: float = 3.0
    ) -> torch.FloatTensor:
