...
class ServiceWanRequestInfo(ServiceRequestInfo):
    def __init__(self, data_json: Dict[str, str]) -> None:
        if "wan" not in data_json:
            ...
            return
        data_wan_json = data_json["wan"]
        self.gpu_model = data_wan_json["gpu"]
        self.dtype = data_wan_json["dtype"]
        self.ring_size = data_wan_json["ring_size"]
        self.ulysses_size = data_wan_json["ulysses_size"]
        self.world_size = data_wan_json["world_size"]
        self.torch_compile = data_wan_json["torch_compile"]
        gen_timer = data_wan_json["gen_timer"][-1]
...
if __name__ == "__main__":
    ...
    url = f"http://{container_ip}:{container_port}/wan"
    logging.info(f"Using Wan at URL: {url}")
    ...
    # Print CSV header
    server_csv_header = ServiceWanRequestInfo.get_csv_header()
            ...
            for test_frames in TEST_NUM_FRAMES:
                for test_w, test_h in TEST_VIDEO_SIZES:
                    ...
                    if response.ok and "video/mp4" in response.headers.get("Content-Type", ""):
                        ...
                        logging.error("Failed to get video from response: %s",
